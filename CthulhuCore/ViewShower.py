...
class ViewInfoChanger(TableInfoChanger):
    ...
    # нулевым элементом является таблица источник этого поля(справочник, где есть первичный ключи,
    # первым строка в которой указаны столбцы данной таблицы,
    # сначала внешний ключ таблицы источника, остальные это атрибуты для визуализации,
    # вторым элементом поле для поиска по шаблону LIKE,
    # третьим элементом поле, на которое ссылается внешний ключ(основе view), поле, которое оборачивает комбобокс
