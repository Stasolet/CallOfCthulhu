...
class WorkersEditor(ViewInfoChanger):
    def __init__(self, header, info, parent: ViewShower):
        ...
        way = self.db.execute(f"SELECT код_параметра, название_параметра, корректировка_нижнего_порога, корректировка_верхнего_порога FROM `корректировки_view` where `табельный_номер` = {self.worker_id}")
    ...
    def push_slave_changes(self):
        ...
        try:
            ...
            self.db.execute("delete from `корректировки` where табельный_номер = %s", (self.worker_id,))
